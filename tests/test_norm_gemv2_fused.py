...
def _norm_gemv2_split_bytes(n0: int, n1: int, k_size: int) -> int:
    ...
    return rms_norm_bytes + gemv0_bytes + gemv1_bytes
...
def test_norm_gemv2_matches_three_paths() -> None:
    ...
    for n0, n1, k_size in SHAPES:
        ...
        out0, out1 = esimd_norm_gemv2_fp8_pert(
            hidden,
            norm_weight,
            weight0_fp8,
            scale0,
        ...
        fused_output = torch.cat([out0, out1], dim=1)
        ...
        combined_output = torch.empty(1, total_n, dtype=torch.float16, device=DEVICE)
        esimd_norm_gemv_fp8_pert(
            hidden,
            norm_weight,
            combined_weight,
        ...
        fused_vs_split0 = (out0.float() - split_out0.float()).abs()
        fused_vs_split1 = (out1.float() - split_out1.float()).abs()

        assert fused_vs_combined.max().item() <= 5e-2
        assert fused_vs_combined.mean().item() <= 5e-3
        assert max(fused_vs_split0.max().item(), fused_vs_split1.max().item()) <= 5e-2
        assert max(fused_vs_split0.mean().item(), fused_vs_split1.mean().item()) <= 5e-3
...
def benchmark_norm_gemv2_three_paths() -> None:
    ...
    for n0, n1, k_size in SHAPES:
        ...
        fused_out0 = torch.empty(1, n0, dtype=torch.float16, device=DEVICE)
        fused_out1 = torch.empty(1, n1, dtype=torch.float16, device=DEVICE)
        combined_output = torch.empty(1, total_n, dtype=torch.float16, device=DEVICE)
        normed = torch.empty_like(hidden)
        split_out0 = torch.empty(1, n0, dtype=torch.float16, device=DEVICE)
        split_out1 = torch.empty(1, n1, dtype=torch.float16, device=DEVICE)
