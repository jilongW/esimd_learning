/* fp8_GEMV_ptl.h — PTL-optimized FP8 GEMV with K_SPLIT and K-tail handling.
 *
...
template<int VL>
SYCL_ESIMD_FUNCTION inline simd<float, VL> fp8_dequant_ptl(
...
/* GEMV per-tensor scale, PTL-tuned with K_SPLIT and tail handling.
 *
...
struct GEMV_fp8_pert_ptl_kernel {
    const fp16*    input;
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        for (; k < ks + kp_full; k += VL) {
            ...
            simd<float, VL> wf = fp8_dequant_ptl<VL>(raw, fp8_mode);
...
struct GEMV_fp8_pert_ptl_tail_kernel {
    const fp16*    input;
    const uint8_t* weight;
    const float*   scale_ptr;
    fp16*          output;
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        for (int k = ks; k < ks + kp_full; k += VL_BIG) {
            ...
            simd<float, VL_BIG> wf = fp8_dequant_ptl<VL_BIG>(raw, fp8_mode);
        ...
        if constexpr (VL_TAIL > 0) {
            ...
            simd<float, VL_TAIL> wf_t = fp8_dequant_ptl<VL_TAIL>(raw_t, fp8_mode);
 ...
 * Goal: maximize parallelism. PTL has ~480 hardware threads.
 * Strategy:
 *   - Pick K_SPLIT so that N × K_SPLIT >= 960 (target full saturation).
 *   - K_SPLIT must divide K (kp = K/K_SPLIT).
 *   - VL_BIG: prefer 256 / 128, or whatever makes (kp / VL_BIG) ≥ 2 with a
 *     small tail.
 */
inline void select_ptl(uint32_t N, uint32_t K, int& vl_big, int& vl_tail, int& ks) {
    ...
    int target_ks = 1;
    if (N * 8 <= 480) target_ks = 8;
    else if (N * 4 <= 480) target_ks = 4;
    else if (N * 2 <= 480) target_ks = 2;
    else target_ks = 1;
...
inline void GEMV_fp8_pert_ptl_host(
    ...
    uint32_t N, uint32_t K, int fp8_mode, sycl::queue& q)
{
    ...
    select_ptl(N, K, vl_big, vl_tail, ks);

    uint32_t global = N * ks;
    uint32_t local  = ks;

    #define LAUNCH_NOTAIL(V, KS)         q.submit([&](sycl::handler& h) {             h.parallel_for(sycl::nd_range<1>(global, local),                 GEMV_fp8_pert_ptl_kernel<V, KS>{p_in, p_w, p_sc, p_out, (int)N, (int)K, fp8_mode});         });
    #define LAUNCH_TAIL(V, T, KS)         q.submit([&](sycl::handler& h) {             h.parallel_for(sycl::nd_range<1>(global, local),                 GEMV_fp8_pert_ptl_tail_kernel<V, T, KS>{p_in, p_w, p_sc, p_out, (int)N, (int)K, fp8_mode});         });
