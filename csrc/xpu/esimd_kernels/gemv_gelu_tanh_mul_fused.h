...
struct GemvGeluTanhMul_fp8_pert_kernel_256_1 {
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        output[n] = fp16(gelu_tanh_scalar_gemv(first) * second);
    }
};
...
struct GemvGeluTanhMul_fp8_pert_kernel {
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        if constexpr (K_SPLIT > 1) {
            ...
            constexpr int cache_bytes = CACHE_X_NORM ? (GEMV_GELU_TANH_MUL_SLM_CACHE_K * sizeof(fp16)) : 0;
        ...
        if constexpr (K_SPLIT == 1) {
            output[n] = fp16(gelu_tanh_scalar_gemv(my_sum0) * my_sum1);
        } else {
            ...
            if (lid == 0) {
                ...
                output[n] = fp16(gelu_tanh_scalar_gemv(sum0) * sum1);
            }
        }
    }
};
    ...
    sycl::queue& q)
{
    ...
    if (vl == 256 && ks == 1) {
        q.submit([&](sycl::handler& cgh) {
            cgh.parallel_for(
                ...
                GemvGeluTanhMul_fp8_pert_kernel_256_1{
                    x_ptr,
                    gemv_weight,
                    gemv_scale,
                    output,
    ...
    #define LAUNCH_GEMV_GELU_TANH_MUL(V, S, CACHE_X_NORM_FLAG) \
        q.submit([&](sycl::handler& cgh) { \
            cgh.parallel_for( \
                sycl::nd_range<1>(global, local), \
                GemvGeluTanhMul_fp8_pert_kernel<V, S, CACHE_X_NORM_FLAG>{ \
